...
typedef struct {
    mm_sleep_status_t status;     // 当前休眠状态 // Current sleep status
    mm_sleep_status_t laststatus; // 上次休眠状态 // Last sleep status
    mm_sleep_level_t level;       // 当前休眠等级 // Current sleep level
    mm_wakeupcd_t wakeupcd;       // 唤醒条件 // Wakeup condition
    uint32_t timestamp;           // 记录每次切换状态的时间戳 // Timestamp for each state switch
    uint32_t key_press_time;      // 按键上次按下时间 // Last key press time

    bool (*is_allow_timeout)(void);  // 满足超时休眠的条件 // Condition for timeout sleep
    bool (*is_allow_lowpower)(void); // 满足低电量休眠的条件 // Condition for low power sleep
    bool (*is_allow_shutdown)(void); // 满足关机休眠的条件 // Condition for shutdown sleep

    bool (*is_allow_presleep)(void); // 满足预休眠条件 // Condition for pre-sleep
    bool (*is_allow_stop)(void);     // 满足进入STOP条件 // Condition for entering STOP
    bool (*is_allow_wakeup)(void);   // 满足进入唤醒条件 // Condition for entering wakeup
} mm_sleep_info_t;
...
static mm_sleep_info_t mm_sleep_info = {
    .status = mm_state_normal,        // 默认正常工作模式 // Default normal working mode
    .laststatus = mm_state_normal,        // 默认正常工作模式 // Default normal working mode
    .level = mm_sleep_level_timeout, // 默认超时休眠 // Default timeout sleep
    .wakeupcd = mm_wakeup_none,         // 默认无唤醒方式 // Default no wakeup method
    .timestamp = 0x00,                   // 时间戳默认为0 // Default timestamp 0
    .key_press_time = 0x00,                   // 上次按键按下时间戳 // Last key press timestamp
...
bool mm_modeio_detection(bool update, uint8_t* mode) {
    ...
    *mode = mio_none;
    ...
    #ifdef MM_MODE_SW_PIN // 用来检测无线和有线模式的PIN // Used to detect wireless and wired mode PIN
    #    ifndef MM_USB_MODE_STATE
    #        define MM_USB_MODE_STATE 0 // 默认低电平为USB状态(有线状态) // Default low level is USB state (wired state)
    #    endif
...
__WEAK bool mm_is_to_sleep(void) {
    ...
    case mm_sleep_level_shutdown: {
        ...
        // 低电和关机时不判断按键是否抬起 // For low power and shutdown, do not check if key is released
...
void mm_set_sleep_state(mm_sleep_status_t state) {
    ...
    mm_sleep_manual = false; // 防止唤醒后立即又休眠 // Prevent immediate sleep after wakeup
    bts_send_vendor(v_query_vol); // 查询一次电量信息 // Query battery info once
...
// 超时无操作休眠条件，面向用户函数 // Timeout sleep condition, user-facing function
__WEAK bool mm_is_allow_timeout_user(void) {
    return true;
}

// 超时无操作休眠条件，返回true时系统进入休眠 // Timeout sleep condition, system enters sleep if returns true
__WEAK bool mm_is_allow_timeout(void) {
    ...
    // USB模式并且枚举成功状态，更新超时计数器 // In USB mode and enumerated, update timeout counter
    if ((mm_eeconfig.devs == DEVS_USB) && (USB_DRIVER.state == USB_ACTIVE)) {
        mm_update_key_press_time();
    }
...
// 低电量休眠条件，返回true时系统进入休眠 // Low power sleep condition, system enters sleep if returns true
__WEAK bool mm_is_allow_lowpower(void) {
    return false;
}

// 设定低电量休眠 // Set low power sleep
__WEAK void mm_set_lowpower(bool state) {
    (void)state;
}

// 关机条件，返回true时系统进入休眠 // Shutdown condition, system enters sleep if returns true
__WEAK bool mm_is_allow_shutdown(void) {
    return false;
}

// 设定关机状态 // Set shutdown state
__WEAK void mm_set_shutdown(bool state) {
    (void)state;
}

// 预休眠时清除所有按键扫描 // Clear all key scans during pre-sleep
...
bool mm_is_allow_presleep(void) {
    ...
    case mm_sleep_level_shutdown: {
        #ifndef SHUTDOWN_TRIGGER_ON_KEYUP
        // 低电和关机时不判断其它条件，直接允许presleep // For low power and shutdown, allow presleep directly without checking other conditions
        return true;
        #endif
    } break;
    default:
...
bool mm_is_allow_stop(void) {
    ...
    case mm_state_presleep: {
        #ifndef SHUTDOWN_TRIGGER_ON_KEYUP
        // 当处于关机模式时，第一次休眠不判断其它条件 // In shutdown mode, do not check other conditions for the first sleep
    ...
    case mm_state_stop: {
        // 上次切换模式的时间戳和现在的时间相差MM_WAKEUP_DELAY时，允许唤醒 // Allow wakeup if time since last mode switch >= MM_WAKEUP_DELAY
...
bool mm_is_allow_wakeup(void) {
    ...
    switch (mm_sleep_info.laststatus) {
    case mm_state_normal: // 为了可以处理手动唤醒增加此条件 // Add this to allow manual wakeup
    case mm_state_presleep:
    case mm_state_stop:
...
// 2.4G状态回调函数 // 2.4G state callback function
...
static void mm_sleep_task(void) {
    ...
    case mm_state_stop: {
        if (mm_sleep_info.is_allow_stop && mm_sleep_info.is_allow_stop()) {
            mm_sleep_manual = false; // 处理2.4G模式下，在键盘已经休眠过程中发送suspend信号时，导致键盘正常唤醒后又立即休眠的问题 // In 2.4G mode, fix issue where suspend signal during sleep causes immediate re-sleep after wakeup
            ...
            if ((mm_eeconfig.devs != DEVS_USB) || (!mm_eeconfig.charging)) {
                ...
                case mm_sleep_level_shutdown: {
                    if (mm_sleep_info.laststatus != mm_state_stop) {
                        // 低电和关机时清除所有按键并切换到usb模式 // For low power and shutdown, clear all keys and switch to USB mode
                        mm_clear_all_keys();
                        bts_send_vendor(v_usb);
                        while (bts_is_busy()) {}
                    }
                ...
                case mm_sleep_level_timeout: {
                    // 超时休眠时清令模组切换到usb模式 // On timeout sleep, force module to switch to USB mode
            ...
            // 根据唤醒条件进行对应处理 // Handle according to wakeup condition
            switch (mm_get_sleep_level()) {
            case mm_sleep_level_timeout: {
    ...
    case mm_state_wakeup: {
        if (mm_sleep_info.is_allow_wakeup && mm_sleep_info.is_allow_wakeup()) {
            ...
            // 确保是事先休眠过才能允许执行wakeup // Ensure sleep occurred before allowing wakeup
...
__WEAK bool mm_stop_process_timeout(void) {
    ...
    case mm_wakeup_none:
    case mm_wakeup_uart: {
        // UART唤醒不直接唤醒 // Do not wake up directly on UART wakeup
        mm_set_sleep_state(mm_state_stop);
    } break;
    default: {
        // 非UART唤醒时，不判断唤醒条件，直接唤醒 // For non-UART wakeup, wake up directly without checking condition
        mm_set_sleep_state(mm_state_wakeup);
        retval = true;
    } break;
    }
...
__WEAK bool mm_stop_process_lowpower(void) {
    ...
    case mm_wakeup_cable: {
        // 低电量休眠只能插入USB唤醒 // Low power sleep can only be woken up by USB insertion
        mm_set_sleep_state(mm_state_wakeup);
        mm_set_sleep_level(mm_sleep_level_timeout);
...
__WEAK bool mm_stop_process_shutdown(void) {
    ...
    case mm_wakeup_onekey: {
        // 低电量休眠只能插入开机键唤醒, 唤醒后先保持STOP状态，等待长按触发唤醒 // Only power key can wake up from low power sleep, after wakeup stay in STOP state and wait for long press to trigger wakeup
        mm_set_sleep_state(mm_state_stop);
    } break;
    default: {
        mm_set_sleep_state(mm_state_stop);
